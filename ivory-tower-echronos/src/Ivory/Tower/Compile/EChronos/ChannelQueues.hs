...
module Ivory.Tower.Compile.EChronos.ChannelQueues where
...
import qualified Ivory.OS.EChronos.Atomic as A

import Ivory.Tower.Types
import Ivory.Tower.Compile.EChronos.Types

data EChronosChannel area =
  EChronosChannel
    { fch_name      :: String
...
eventGuard :: TaskNode -> EChronosGuard
eventGuard node = EChronosGuard
  { guard_block     = block
  , guard_notify    = notify
  ...
  where
  -- At least 1 to be a valid echronos primitive.
...
eventQueue channelid sizeSing dest = if size < 2 then err else fch
  ...
  err = error ("error: in channel named " ++ name ++ ":\n" ++
               "Tower EChronos Channel must be of size > 1; " ++
               "channels hold one fewer event than their size")
  fch = EChronosChannel
    { fch_name        = unique "echronos_eventQueue"
    , fch_emit        = emit
    , fch_emit_       = emit_
    , fch_receive     = receive
    , fch_initDef     = initDef
  ...
  initName = unique "echronos_eventQueue_init"
