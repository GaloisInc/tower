...
data Unique = Unique
  { unique_userprovided :: String
  , unique_autogenerated :: String
  } deriving (Eq, Show)

type UniqueBuilder = String -> Unique

showUnique :: Unique -> String
showUnique (Unique a b) = a ++ b
...
instance Functor Labeled where
  fmap f (Labeled a u) = Labeled (f a) u
...
data NodeEdges =
  NodeEdges
    { nodees_name        :: Unique
    , nodees_emitters    :: [(ChannelId,  Unique, SymbolName)]
    , nodees_receivers   :: [(ChannelId,  Unique, SymbolName)]
    , nodees_datareaders :: [(DataportId, Unique, SymbolName)]
    , nodees_datawriters :: [(DataportId, Unique, SymbolName)]
    }

lblWOSymbol :: (a , Unique, SymbolName) -> Labeled a
lblWOSymbol (a, u, _) = Labeled a u
-- ease of use / compatibility with legacy
nodest_name :: NodeSt a -> Name
nodest_name = showUnique . nodees_name . nodest_edges
nodest_emitters :: NodeSt a -> [Labeled ChannelId]
nodest_emitters = (map lblWOSymbol) . nodees_emitters . nodest_edges
nodest_receivers :: NodeSt a -> [Labeled ChannelId]
nodest_receivers = (map lblWOSymbol) . nodees_receivers . nodest_edges
nodest_datareaders :: NodeSt a -> [Labeled DataportId]
nodest_datareaders = (map lblWOSymbol) . nodees_datareaders . nodest_edges
nodest_datawriters :: NodeSt a -> [Labeled DataportId]
nodest_datawriters = (map lblWOSymbol) . nodees_datawriters . nodest_edges
...
emptyNodeEdges :: Unique -> NodeEdges
...
data OS =
  OS
    ...
    , os_mkChannel     :: forall area i n
                       ...
                       -> (Def ('[]:->()), ModuleDef)
    -- Timing
    , os_mkPeriodic   :: Integer -> String -> (Period, Def ('[]:->()), ModuleDef)
...
class (Monad m) => BaseUtils m where
  getOS :: m OS
  fresh :: m Int
  freshname :: m UniqueBuilder
  freshname = do
    n <- fresh
    return (builder n)
    where
    builder n u = Unique { unique_userprovided = u
                         , unique_autogenerated = ("_" ++ (show n))}
