...
module Ivory.Tower.Compile.GRTOS
  ( os
  , searchDir
  ) where
...
import Ivory.Tower.Compile.GRTOS.SearchDir (searchDir)
import Ivory.Tower.Compile.GRTOS.MsgQueue
import Ivory.Tower.Compile.GRTOS.EventNotify
import Ivory.Tower.Compile.GRTOS.ISR

import qualified Ivory.OS.GRTOS.Task as Task
import qualified Ivory.OS.GRTOS.Atomic as Atomic
import qualified Ivory.OS.GRTOS.Time as Time
...
time_mod :: Module
time_mod = package "tower_grtos_time" $ do
  incl time_proc
  Time.moddef
