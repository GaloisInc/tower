...
module Ivory.Tower.Compile.GRTOS.EventNotify
  ( EventNotify(..)
  , taskEventNotify
  ) where
...
import qualified Ivory.OS.GRTOS.CountingSemaphore as S
...
taskEventNotify taskname = EventNotify
  ...
  code = do
    inclHeader "grtos_semaphore_wrapper.h"
    sourceDep "grtos_semaphore_wrapper.h"
    sourceDep "grtos_semaphore_wrapper.c"
    incl ini
    incl trigger
