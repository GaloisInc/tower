...
staticAnalysisHandler :: [Mon.CodeBlock] -> [Sym]
staticAnalysisHandler l = nub $ concat $ map analyseCodeBlock l

analyseCodeBlock :: Mon.CodeBlock -> [Sym]
analyseCodeBlock block = 
  (analyseBlock $ Mon.blockStmts $ block) ++ 
  (concat $ map analyseRequire $ Mon.blockRequires block) ++ 
  (concat $ map analyseEnsure $ Mon.blockEnsures block)
